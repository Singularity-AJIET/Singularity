...
type Advisor = PatronLogo | PatronPerson;

// ── Section 1: Advisors (3 containers) ───────────────────────────
...
// ── Section 2: Faculty Coordinators (4 containers) ───────────────
...
export default function AdvisorsSection() {
  return (
    <section id="advisors" className={styles.section}>
      <div className="section">
        ...
        <div className={styles.tier}>
          <div className={`${styles.tierGrid} ${styles.gridAdvisors}`}>
            {ADVISORS.map((p, i) =>
              p.type === "person" ? (
                <PersonCard key={`advisor-${i}`} patron={p} />
              ) : (
                <LogoCard key={`advisor-${i}`} patron={p} />
              )
            )}
          </div>
        </div>

        {/* Section 2: Faculty Coordinators (Heading + 4 containers) */}
        <div className={styles.tier}>
